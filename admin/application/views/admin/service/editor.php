			...
			<div class="panel-body">
				...
				<form method="post" action="<?php echo base_url('service/save'); ?>">
					<div class="form-group">
						...
						<select class="form-control" name="service_category_id" required>
							...
							<?php foreach ($data_service_category as $row) { ?>
								<option value="<?= $row['category_id']; ?>" <?php if ($service_category_id == $row['category_id']) {
																								echo 'selected';
					...
					<div class="form-group">
						...
						<select name="service_city_id" class="form-control" required>
							<option value="">Choose</option>
							<?php
							if (!empty($data_service_city)) {
								foreach ($data_service_city as $city) {
							?>
									<option value="<?= $city['city_id'] ?>" <?= ($service_city_id == $city['city_id']) ? 'selected' : '' ?>><?= $city['city_name'] ?></option>
							<?php
								}
							}
							?>
					...
					<div class="form-group">
						...
						<input type="number" min="0" class="form-control" name="service_cost" value="<?php echo $service_cost; ?>" required>
					</div>
					<!--
					<div class="form-group">
